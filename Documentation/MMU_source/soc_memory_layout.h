...
#define SOC_RESERVE_MEMORY_REGION(START, END, NAME) \
	__attribute__((section(".reserved_memory_address"))) __attribute__((used)) static soc_reserved_region_t reserved_region_##NAME = {START, END};
...
inline static bool IRAM_ATTR esp_ptr_executable(const void *p)
{
	...
	return (ip >= SOC_IROM_LOW && ip < SOC_IROM_HIGH) || (ip >= SOC_IRAM_LOW && ip < SOC_IRAM_HIGH) || (ip >= SOC_IROM_MASK_LOW && ip < SOC_IROM_MASK_HIGH)
